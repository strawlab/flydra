...
class CoordinateProcessor(threading.Thread):
    def __init__(self,main_brain,save_profiling_data,
        ...
        if self.listen_address.has_key('host') and self.listen_address['host']=='0.0.0.0':
            self.listen_address['host']= flydra_core.rosutils.get_node_hostname( rospy.get_name() )
    ...
    def set_new_tracker(self,kalman_model=None):
        # called from main thread, must lock to send to realtime coord thread
        tracker = flydra_core.kalman.flydra_tracker.Tracker(self.reconstructor,
                ...
                for frame_pt_idx in range(n_pts):
                    ...
                    if cam_id in self.cached_calibration_by_cam_id:
                        ...
                        (p1, p2, p3, p4, ray0, ray1, ray2, ray3, ray4,
                         ray5) = flydra_core.reconstruct.do_3d_operations_on_2d_point(
