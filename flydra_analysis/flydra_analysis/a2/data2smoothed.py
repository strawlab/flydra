...
def convert(infilename,
    ...
    with ca.kalman_analysis_context(infilename,
        ...
        uuid=None
        try:
            table_experiment = h5_context.get_pytable_node( 'experiment_info',
                                                            from_2d_file=True )
        ...
        smoothing_flydra_version = h5_context.get_extra_info()['header']['flydra_version']
        ...
        for i,obj_id in enumerate(obj_ids):
            ...
            try:
                rows = h5_context.load_data(obj_id,
                                            dynamic_model_name=dynamic_model_name,
                                            frames_per_second=frames_per_second,
                                            **kwargs)
            except core_analysis.DiscontiguousFramesError:
                warnings.warn('discontiguous frames smoothing obj_id %d, skipping.'%(obj_id,))
                continue
            ...
            allrows.append(rows)
            try:
                qualrows = compute_ori_quality(h5_context,
                                               rows['frame'],
                                               obj_id,
                                               smooth_len=0)
