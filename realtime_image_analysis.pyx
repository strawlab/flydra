...
cdef class RealtimeAnalyzer:
    ...
    def __init__(self, int w, int h):
        self.width = w
        self.height = h
        self.set_roi( 0, 0, self.width-1, self.height-1)
        ...
        CHK( ipp.ippiSet_8u_C1R(0,self.bg_img,self.bg_img_step, self.roi_sz))
        CHK( ipp.ippiSet_8u_C1R(0,self.std_img,self.std_img_step, self.roi_sz))

        CHK( ipp.ippiSet_32f_C1R(0.0,self.mean_image,self.mean_image_step, self.roi_sz))
        CHK( ipp.ippiSet_32f_C1R(0.0,self.std_image,self.std_image_step, self.roi_sz))
    ...
    def set_roi(self,int left, int bottom, int right, int top):
        self.left = left
        self.bottom = bottom
        self.right = right
        self.top = top
        
        self.roi_sz.width = self.right-self.left+1
        self.roi_sz.height = self.top-self.bottom+1
    ...
    def do_work(self, c_numarray._numarray buf):
        ...
        CHK( ipp.ippiAbsDiff_8u_C1R(
            (self.bg_img + self.bottom*self.bg_img_step + self.left), self.bg_img_step,
            (self.im1 + self.bottom*self.im1_step + self.left), self.im1_step,
            (self.im2 + self.bottom*self.im2_step + self.left), self.im2_step, self.roi_sz))
        CHK( ipp.ippiMaxIndx_8u_C1R(
            (self.im2 + self.bottom*self.im2_step + self.left), self.im2_step,
            self.roi_sz, &max_val, &index_x,&index_y))
        # (to avoid big moment arm:) if pixel < .8*max(pixel): pixel=0
        CHK( ipp.ippiThreshold_Val_8u_C1IR(
            (self.im2 + self.bottom*self.im2_step + self.left), self.im2_step,
            self.roi_sz, self._clear_threshold*max_val, 0, ipp.ippCmpLess))

        if max_val < self._diff_threshold:
            x0=-1
            y0=-1
        else:
            #print 'called c_fit_params'
            c_fit_params.fit_params( &x0, &y0, &orientation,
                                     self.roi_sz.width, self.roi_sz.height,
                                     (self.im2 + self.bottom*self.im2_step + self.left),
                                     self.im2_step )
    ...
    def clear_background_image(self):
        ...
        CHK( ipp.ippiSet_8u_C1R( 0,
                                 (self.bg_img + self.bottom*self.bg_img_step + self.left),
                                 self.bg_img_step, self.roi_sz))
        CHK( ipp.ippiSet_8u_C1R( 0,
                                 (self.std_img + self.bottom*self.std_img_step + self.left),
                                 self.std_img_step, self.roi_sz))
        # end of IPP-requiring code
        return
    ...
    def clear_accumulator_image(self):
        ...
        CHK( ipp.ippiSet_32f_C1R( 0.0, 
                                  (self.sum_image + self.bottom*self.sum_image_step/4 + self.left),
                                  self.sum_image_step, self.roi_sz)) 
        CHK( ipp.ippiSet_32f_C1R( 0.0,
                                  (self.sq_image + self.bottom*self.sq_image_step/4 + self.left),
                                  self.sq_image_step, self.roi_sz))
        # end of IPP-requiring code
        return
    ...
    def convert_accumulator_to_bg_image(self,int n_bg_samples):
        ...
        CHK( ipp.ippiMulC_32f_C1R(
            ...
            (self.mean_image + self.bottom*self.mean_image_step/4 + self.left),
            self.mean_image_step, self.roi_sz))
        CHK( ipp.ippiConvert_32f8u_C1R(
            (self.mean_image + self.bottom*self.mean_image_step/4 + self.left),
            self.mean_image_step,
            (self.bg_img + self.bottom*self.bg_img_step + self.left),
            self.bg_img_step, self.roi_sz, ipp.ippRndNear ))
        ...
        CHK( ipp.ippiMulC_32f_C1R(
            ...
            (self.std_image + self.bottom*self.std_image_step/4 + self.left),
            self.std_image_step, self.roi_sz))
        ...
        CHK( ipp.ippiConvert_32f8u_C1R(
            ...
            self.std_image_step,
            (self.std_img + self.bottom*self.std_img_step + self.left),
            self.std_img_step, self.roi_sz, ipp.ippRndNear ))
        # end of IPP-requiring code
        return
