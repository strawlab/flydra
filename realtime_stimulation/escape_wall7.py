...
log_file = open(time.strftime( 'escape_wall7_%Y%m%d_%H%M%S.log' ), mode='wb')
#log_file = sys.stdout
...
while not quit_now:
    ...
    swap_buffers()
    if echo_flip_time:
        outgoing_UDP_socket.sendto(repr(cf),0,('192.168.1.151',28933))
    frame_timer.tick()
