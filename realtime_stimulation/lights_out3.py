...
if 1:
    ...
    LIGHTS_OFF = '0'
    LIGHTS_STROBE = 'S'
    LIGHTS_STROBE_ONCE = 'S1'
    LIGHTS_FOCAL_ONCE = 'F1'

##light_val_dict = {LIGHTS_OFF:LIGHTS_OFF,
##                  LIGHTS_STROBE_ONCE:LIGHTS_STROBE_ONCE,
##                  LIGHTS_FOCAL_ONCE:LIGHTS_FOCAL_ONCE}
lights_off_all = [LIGHTS_OFF,
                  LIGHTS_STROBE_ONCE,
                  LIGHTS_FOCAL_ONCE,]
#:LIGHTS_FOCAL_ONCE}0.0, 1.0, 0.5]
lights_off_idx = 0
...
log_file = open(time.strftime( 'lights_out3_%Y%m%d_%H%M%S.log' ), mode='wb')
#log_file = sys.stdout
...
while not quit_now:
    ...
    if (status == 'armed' and trigger_corrected_framenumber!=cf and
        ...
        lights_off_key = lights_off_all[lights_off_idx]
        cmd = lights_off_key
        ser.write(cmd)
        print cmd
        timetime = time.time()
        log_file.write('%d %s %s\n'%(cf,repr(timetime),cmd))
        log_file.flush()
        mode_start_time = now
