...
class DistortionMesh:
    def __init__(self, helper, undistorted_bounds_lbrt, xdim = 200, ydim=200 ):
        ...
        Xu, Yu = numpy.mgrid[l:r:(xdim*1j), b:t:(ydim*1j)]

        shape = Xu.shape
        distorted_x = numpy.zeros( shape, dtype=numpy.float32 )
        distorted_y = numpy.zeros( shape, dtype=numpy.float32 )
        ...
        self._distorted_x_mesh = distorted_x
        self._distorted_y_mesh = distorted_y

    def undistort_points(self, distorted_xs, distorted_ys ):
        undistorted_x = self._interp_x( distorted_xs, distorted_ys )
        undistorted_y = self._interp_y( distorted_xs, distorted_ys )
    ...
    def undistort_image(self, distorted_image, interpolation='nearest' ):
        ...
        if not len(distorted_image.shape)==2:
            raise NotImplementedError('only grayscale images for now')

        color_interp = []
        valid = 0
        for (x,y) in zip(numpy.ravel(self._distorted_x_mesh),
                         numpy.ravel(self._distorted_y_mesh)):
            xr = int(numpy.round(x))
            yr = int(numpy.round(y))
            if (0 <= xr < distorted_image.shape[1] and
                0 <= yr < distorted_image.shape[0]):

                color = distorted_image[yr,xr]
                valid+=1
            else:
                color = nan
            color_interp.append( color )
        print 'valid',valid

        interp = self._tri.nn_interpolator(color_interp)

        newx_vals = numpy.arange( distorted_image.shape[1] )
        newy_vals = numpy.arange( distorted_image.shape[0] )

        newx, newy = numpy.meshgrid( newx_vals, newy_vals )
        new_image_data = interp( numpy.ravel( newx ), numpy.ravel( newy ) )
        new_image_data.shape = distorted_image.shape
