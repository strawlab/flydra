...
class EquivalentObjectFinder:
    ...
    def find_equiv( self, src_obj_id, mean_distance_maximum=None ):
        ...
        src_rows = self.ca.load_data( src_obj_id, self.src_h5,
        ...
        for obj_id in candidate_obj_id:
            ...
            dst_rows = self.ca.load_data( obj_id, self.dst_h5,
                                          use_kalman_smoothing=False )
        ...
        if not np.isfinite(best_error):
            ...
            if ((mean_distance_maximum is None) or
                (best_error <= mean_distance_maximum)):
