...
if 1:
    ...
    if 1:
        ...
        try:
            ...
            allframes = data2d.read(field='frame',
                                    flavor='numpy')
...
#                     list(range(4000000,4001000)) +
        ...
        for chunknum in [0,500,1000,2000]:
            ...
            chunk = data2d.read(start=chunkstart,
                                stop=chunkstop,
                                flavor='numpy')
    ...
    if 1:
        for ref_idx in range(len(ucamns)):
            ...
            for i in range(n_cams):
                ...
                pylab.plot( framenumbers, dvmsec, '.-',
                            label=camn2cam_id[ucamns[idx]])
                miny = dvmsec.min()
                ax.set_ylim((miny-.5,miny+maxrange+.5))
                pylab.ylabel('msec')
                pylab.xlabel('frame')
        ...
        l2dist = numpy.sqrt(numpy.sum((diff_vectors[1:] - diff_vectors[:-1])**2,axis=1))
