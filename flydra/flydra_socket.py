...
class FlydraTransportSender:
    def __init__(self,destination_addr):
        ...
        self._sockobj = socket.socket(self._dest.family,
                                      socket.SOCK_DGRAM)
...
def does_host_require_DNS(host,port=None,family=0):
    try:
        addrs = socket.getaddrinfo(host, port,
                                   family, 0, 0, socket.AI_NUMERICHOST)
...
def test_flydra_ip_sockets():
    for host,port,require_DNS_expected in [ ('localhost',          None, True),
