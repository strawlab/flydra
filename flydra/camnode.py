    ...
    def __init__(self,
        ...
        for cam_no in range(num_cams):
            ...
            with cam._hack_acquire_lock():
                ...
                left,top,width,height = cam.get_frame_roi() # XXX ADS LOCK
                del left,top
                globals = self.globals[cam_no] # shorthand
                ...
                if 1:
                    # trigger modes
                    N_trigger_modes = cam.get_num_trigger_modes()  # XXX ADS LOCK
                    if options.show_cam_details:
                        print '  %d available trigger modes:'%N_trigger_modes
                        for i in range(N_trigger_modes):
                            mode_string = cam.get_trigger_mode_string(i)  # XXX ADS LOCK
                            print '  mode %d: %s'%(i,mode_string)
                ...
                num_props = cam.get_num_camera_properties()  # XXX ADS LOCK
                prop_names = []
                for prop_num in range(num_props):
                    props = cam.get_camera_property_info(prop_num)  # XXX ADS LOCK
                    current_value,auto = cam.get_camera_property( prop_num )  # XXX ADS LOCK
                    ...
                    if props['has_manual_mode']:
                        if force_manual or min_value <= new_value <= max_value:
                            try:
                                ...
                                cam.set_camera_property( prop_num, new_value, 0 )  # XXX ADS LOCK
                            except:
                ...
                try:
                    scalar_control_info['trigger_mode'] = cam.get_trigger_mode_number()  # XXX ADS LOCK
                ...
                scalar_control_info['max_framerate'] = cam.get_framerate()  # XXX ADS LOCK
                ...
                if not DISABLE_ALL_PROCESSING:
                    if 0:
                        ...
                        cam.get_max_height()  # XXX ADS LOCK
                        l,b,w,h = cam.get_frame_roi()  # XXX ADS LOCK
                        r = l+w-1
                        t = b+h-1
                        lbrt = l,b,r,t
                        cam_processor = ProcessCamClass(
                            ...
                            framerate=None,
                            lbrt=lbrt,
                            max_height=cam.get_max_height(),  # XXX ADS LOCK
                            max_width=cam.get_max_width(),  # XXX ADS LOCK
                            globals=globals,
                            options=options,
