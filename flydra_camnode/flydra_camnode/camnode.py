...
#        self._hw_roi_frame = hw_roi_frame # make accessible to other code
        ...
        while 1:
            ...
            with camnode_utils.use_buffer_from_chain(self._chain) as chainbuf:
                ...
                data = struct.pack(flydra_core.common_variables.recv_pt_header_fmt,
                                   self.cam_id,
                                   timestamp,cam_received_time,
                                   framenumber,len(points),n_frames_skipped)
                for point_tuple in points:
                    try:
                        data = data + struct.pack(flydra_core.common_variables.recv_pt_fmt,*point_tuple)
                    except:
...
class AppState(object):
    ...
    def __init__(self,
        ...
        if benchmark:
            ...
            self.main_brain.log_message(rospy.get_name(),time.time(),'using flydra version %s' % flydra_camnode.version.__version__)
        ##################################################################
        #
...
def parse_args(benchmark, cmdline_args):
    parser = optparse.OptionParser(usage="%prog [options]",
                          version="%prog "+flydra_camnode.version.__version__)
